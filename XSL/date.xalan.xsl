...
<xsl:stylesheet version="1.0"
                ...
                >
<xalan:component prefix="date" functions="formatDate date dateTime">
		...
		for (var i=0; i < format.length;){
			...
			switch(format.substr(i, 1)){
				...
				case "M"://        month in year           (Text &amp; Number)     July &amp; 07
                    ...
                    switch (i-s) {
                        case 1:
						    retString += oDate.getMonth()+1;
					        break;
                        case 2:
						    retString += pad(oDate.getMonth()+1);
                            break;
                        case 3:
                            retString += _monthAbbreviation(oDate.getMonth(), dateF, ctx);
                            break;
                        default:
				...
				case "E"://        day in week             (Text)              Tuesday
					...
					if (i-s <= 3){
						retString += _dayName(oDate.getDay(), dateF, ctx).substring(0, i-s);
					}else{
						retString += _dayName(oDate.getDay(), dateF, ctx);
					}
					break;
			...
			for (var i=0,j=0; i < format.length & oDate.toString() != "NaN";){
				...
				switch(format.substr(i, 1)){
					...
					case "M"://        month in year           (Text &amp; Number)     July &amp; 07
						...
						if (i-s <= 2){
							oDate.setMonth(date.substr(s,j-s)-1,1);
						}else{
