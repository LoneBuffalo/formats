...
<xsl:variable name="editionID" select="/CLIPSHEET/EDITION/@ID" />
	...
	<tr>
		...
		<td>
	 		<div>
				...
				<xsl:choose>
					<xsl:when test="SRCURL/@REDIRECT=1"><xsl:call-template name="do-link-headline" /></xsl:when>
					<xsl:otherwise><xsl:call-template name="do-headline" /></xsl:otherwise>
				</xsl:choose>
...
<xsl:template match="ARTICLE" mode="relatedArticle">
	<div class="relatedArticle">
		...
		<span class="relatedTitle">
			<xsl:choose>
				<xsl:when test="(SRCURL/@REDIRECT=1)"><xsl:call-template name="do-link-related-headline" /></xsl:when>
				<xsl:otherwise><xsl:call-template name="do-related-headline" /></xsl:otherwise>
			</xsl:choose>
		</span>
...
<xsl:template name="do-link-headline">
	<a href="{SRCURL}" target="_blank">
		...
		&#xa0;<span class="articlePublication" style="color:red;font-weight:normal;">Internet</span>
</xsl:template>

<xsl:template name="do-headline">
...
<xsl:template name="do-plain-headline">
		<xsl:value-of disable-output-escaping="yes" select="HEADLINE" />
		<xsl:apply-templates select="LANGGRPH">
				<xsl:with-param name="LangImg" select="LANGGRPH" />
		</xsl:apply-templates>
...
<xsl:template name="do-link-related-headline">
	<a href="{SRCURL}" target="_blank">
		<xsl:value-of disable-output-escaping="yes" select="HEADLINE" />
	</a>
	...
	&#xa0;<span class="relatedPublication" style="color:red;font-size:90%;">Internet</span>
</xsl:template>

<xsl:template name="do-related-headline">
	<a href="{$webroot}story.cfm?story_id={@ID}" target="_blank">
		<xsl:value-of disable-output-escaping="yes" select="HEADLINE" />
	</a>
	<span class="relatedPublication"> - <xsl:value-of disable-output-escaping="yes" select="PUBLICATION" /></span> 
...
<xsl:template name="do-plain-related-headline">
	<xsl:value-of disable-output-escaping="yes" select="HEADLINE" />
</xsl:template>
