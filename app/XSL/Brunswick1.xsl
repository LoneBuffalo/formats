...
<xsl:variable name="formatName">Brunswick 1 (Lending Club)</xsl:variable>
...
<!--	<xsl:choose>
	<xsl:when test="SRCURL/REDIRECT!=1">
	<a href="#DUMP"><xsl:value-of disable-output-escaping="yes" select="HEADLINE" /></a>
	</xsl:when>
	<xsl:otherwise>
	<a href="{SRCURL}"><xsl:value-of disable-output-escaping="yes" select="HEADLINE" /></a>
	</xsl:otherwise>
	</xsl:choose>
	<div style="margin:1em;margin-top:0em;"><xsl:apply-templates select="ABSTRACT" /></div> -->
	</li>
