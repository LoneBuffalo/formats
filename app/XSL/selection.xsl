...
<xsl:variable name="sectList" select="/CLIPSHEET/SECTION[ID=$selection[not(TOPSTORY!=0)]/SECTIONID]" />
...
<xsl:template match="/">
	...
	<xsl:copy-of select="/CLIPSHEET/KEYWORDS" />
...
<xsl:template match="SECTION">
	...
	<xsl:apply-templates select="$selection[SECTIONID=$sectID][not(TOPSTORY!=0)][not(PARENT!=0)]">
		<xsl:with-param name="sectOrd" select="position()" />
		<xsl:sort select="ORDINAL" data-type="number" />
	</xsl:apply-templates>
	<xsl:apply-templates select="$orphans[SECTIONID=$sectID]">
